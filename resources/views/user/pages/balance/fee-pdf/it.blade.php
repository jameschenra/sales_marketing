...
<body>
    <div class="container">
        <table style="margin-left: auto; margin-right: auto" width="550">
            ...
            <tr>
                <td>
                    Ordine n. {{ $transaction->book_id }}<br>
                    Fattura n. {{ $transaction->id }}<br>
                    Data {{ date('d-m-Y', strtotime($transaction->getOriginal('created_at'))) }}
                </td>
            </tr>
            <tr>
                <td align="right">
                    ...
                    P.IVA {{ $billingInfo->invoice_vat_id }}
                </td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    @endif
                    <br />
                    Numero di servizi pagati online : {{ $book->number_of_booking }}<br />
                    Prezzo unitario : @if ($book->price == 0)
            ...
            <tr>
                <td align="right">
                    @if ($billingInfo->company_type_id == CompanyType::TAX_VAT)
                        IMPONIBLE Euro {{ number_format($transaction->amount * 0.78, 2) }}<br /><br />
                        IVA (22%) Euro {{ number_format($transaction->amount * 0.22, 2) }}<br /><br />
                        TOTALE Fattura Euro {{ number_format($transaction->amount, 2) }}
                    @elseif($billingInfo->company_type_id == CompanyType::TAX_UE_VAT)
                        IMPORTO Euro {{ number_format($transaction->amount, 2) }}<br /><br />
                        TOTALE Fattura Euro {{ number_format($transaction->amount, 2) }}<br />
                        Operazione fuori campo IVA ex art. 7-ter del DPR n. 633/1972.
                    @else
                        IMPORTO Euro {{ number_format($transaction->amount, 2) }}<br /><br />
                        TOTAL Fattura Euro {{ number_format($transaction->amount, 2) }}<br />
                        Operazione fuori campo IVA articolo 9 del Dpr 633/1972
                    @endif
                </td>
            </tr>
        </table>
