            ...
            <tr>
                <td align="right">
                    @if ($billingInfo->company_type_id == CompanyType::TAX_VAT)
                        ...
                        TOTAL Factura Euro {{ number_format($transaction->amount, 2) }}
                    @elseif($billingInfo->company_type_id == CompanyType::TAX_UE_VAT)
                        CANTIDAD Euro {{ number_format($transaction->amount, 2) }}<br /><br />
                        TOTAL Factura Euro {{ number_format($transaction->amount, 2) }}<br />
                        Operación fuera del ámbito del IVA ex art. 7-ter del DPR n. 633/1972.
                    @else
                        CANTIDAD Euro {{ number_format($transaction->amount, 2) }}<br /><br />
                        TOTAL Factura Euro {{ number_format($transaction->amount, 2) }}<br />
                        Operación fuera del ámbito del IVA Artículo 9 del Dpr 633/1972
                    @endif
                </td>
            </tr>
        </table>
