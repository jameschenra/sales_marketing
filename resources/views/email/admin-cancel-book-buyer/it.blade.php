...
@section('email-content')
    ...
    <p>a seguito della tua richiesta abbiamo cancellato il tuo appuntamento.</p><br />
    ...
    @if (!empty($bookingTransaction))
        <p>
            <b>Operazione pagamento n.</b> {{ $bookingTransaction->id }}
        </p>
    @endif
    <br><br>
    @if ($book->is_paid_online == 1 && $book->price > 0)
        <p>
            Per effettuare una nuova prenotazione puoi accedere
            direttamente da <b><a href="{{ URL::route('user.auth.login') }}">qui</a></b>. Puoi prelevare la somma
            che ti è stata rimborsata dalla tua pagina del <b><a href="{{ route('user.balance.show') }}">Saldo</a></b>.</p>
    @else
        <p>
            Per effettuare una nuova prenotazione puoi accedere direttamente da <b><a href="{{ URL::route('user.auth.login') }}">qui</a></b>.
        </p>
    @endif
