@php
    ...
    if (isset($profCategory)) {
        $name_en = old('name_en', $profCategory->name_en);
        $name_es = old('name_es', $profCategory->name_es);
        $name_it = old('name_it', $profCategory->name_it);
        $icon = old('icon', $profCategory->icon);
        $description_en = old('description_en', $profCategory->description_en);
        $description_es = old('description_es', $profCategory->description_es);
        $description_it = old('description_it', $profCategory->description_it);
    } else {
        $name_en = old('name_en');
        $name_es = old('name_es');
        $name_it = old('name_it');
...
<div class="row justify-content-center">
    <div class="col-lg-8">
        ...
        <div class="card">
            <form method="POST" action="{{ route('admin.profcat.store') }}">
                @csrf
                @isset($profCategory)
                    <input type="hidden" name="profession_category_id" value="{{ $profCategory->id }}" />
                @endisset
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ route('admin.profcat.index') }}" class="btn btn-light-success font-weight-bold">@lang('main.Back')</a>
                </div>
            </form>
        </div>
    </div>
