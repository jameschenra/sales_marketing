    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title w-100 justify-content-between">
                ...
                <a href="{{ route('admin.profcat.create') }}" class="btn btn-primary"><i class="flaticon2-plus-1"></i>@lang('main.Create')</a>
            </div>
        </div>
        ...
        <div class="card-body">
            <table class="table table-bordered table-striped" id="kt_datatable">
                ...
                <tbody>
                    @foreach ($profCategories as $key => $value)
                        <tr>
                            <td>{{ ((Request::has('page') ? Request::get('page') : 1) - 1 ) * PAGINATION_SIZE + ($key + 1) }}</td>
                            <td>{{ $value->name }}</td>
                            <td class="text-center"><i class="{{ $value->icon }}"></i></td>
                            <td>{{ count($value->professions) }}</td>
                            <td>{{ $value->description }}</td>
                            <td>{{ $value->created_at }}</td>
                            <td>
                                <a href="{{ route('admin.profcat.edit', $value->id) }}" class="btn btn-sm btn-info">
                                    <span class="glyphicon glyphicon-edit"></span> {{ trans('main.Edit') }}
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('admin.profcat.delete', $value->id) }}" class="btn btn-sm btn-danger" id="btn-delete-item">
            ...
            <br />
            <div class="float-right">{{ $profCategories->links() }}</div>
