...
class PostCategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $param['categories'] = PostCategory::paginate(PAGINATION_SIZE);

        return view('admin.pages.post-category.index', $param);
    }
    ...
    public function create()
    {
        ...
        return view('admin.pages.post-category.form')->with($param);
    }
    ...
    public function edit($id)
    {
        ...
        $param['icons'] = $this->icons;
        $param['postCategory'] = PostCategory::find($id);
    ...
    public function store(Request $request)
    {
        foreach (\LaravelLocalization::getSupportedLocales() as $localeCode => $properties) {
            $nameCtrl = 'name' . (($localeCode == 'en') ? '' : $localeCode);
            $inputval = $request->input($nameCtrl);
            if (!empty($inputval)) {
                $checkCat = PostCategory::where($nameCtrl, $inputval)->first();
            }

            if (isset($checkCat->id) && !$request->has('post_category_id')) {
        ...
        if ($validator->fails()) {
            ...
            if ($request->has('post_category_id')) {
                $id = $request->input('post_category_id');
            ...
            foreach (\LaravelLocalization::getSupportedLocales() as $localeCode => $properties) {
                $nameCtrl = 'name' . (($localeCode == 'en') ? '' : $localeCode);
                $descCtrl = 'description' . (($localeCode == 'en') ? '' : $localeCode);
                $postCategory->$nameCtrl = $request->input($nameCtrl);
                $postCategory->$descCtrl = $request->input($descCtrl);
            }
            ...
            $postCategory->icon = $request->input('icon');
            $postCategory->save();
