...
class ProfCategoryController extends Controller
{
    ...
    public function __construct(Request $request)
    {
        ...
        $this->icons = ['fa-automobile', 'fa fa-bank', 'fa fa-behance', 'fa fa-behance-square', 'fa fa-bomb', 'fa fa-building',
    ...
    public function index()
    {
        ...
        $param['profCategories'] = ProfessionCategory::orderBy('name_en')->paginate(PAGINATION_SIZE);

        return view('admin.pages.profession-category.index')->with($param);
    }
    ...
    public function create()
    {
        ...
        $param['icons'] = $this->icons;

        return view('admin.pages.profession-category.form')->with($param);
    }
    ...
    public function edit($id)
    {
        ...
        $param['icons'] = $this->icons;
        $param['profCategory'] = ProfessionCategory::find($id);
    ...
    public function store(Request $request)
    {
        foreach (\LaravelLocalization::getSupportedLocales() as $localeCode => $properties) {
            ...
            if (!empty($inputval)) {
                $checkCat = ProfessionCategory::where($nameField, $inputval)->first();
            }

            if (isset($checkCat->id) && !$request->has('profession_category_id')) {
                return redirect()->back()->withErrors(['msg' => 'main.Category Already Added']);
            }
        }
        ...
        if ($validator->fails()) {
            ...
            if ($request->has('profession_category_id')) {
                $id = $request->input('profession_category_id');
                $profCategory = ProfessionCategory::find($id);
            } else {
                $profCategory = new ProfessionCategory();
            }
            foreach (\LaravelLocalization::getSupportedLocales() as $localeCode => $properties) {
                $nameField = 'name_' . $localeCode;
                $descCtrl = 'description_' . $localeCode;
                $profCategory->$nameField = $request->input($nameField);
                $profCategory->$descCtrl = $request->input($descCtrl);
            }

            // resluggify
            $profCategory->slug = null;
            $profCategory->save();

            $profCategory->icon = $request->input('icon');
            $profCategory->save();

            session()->flash('message', ['type' => 'success', 'msg' => trans('main.Category has been saved successfully')]);

            return redirect()->route('admin.profcat.index');
        }
    }

    public function delete($id) {
        try {
            ProfessionCategory::find($id)->delete();
